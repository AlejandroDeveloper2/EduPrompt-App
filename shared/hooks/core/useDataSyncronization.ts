...
const useDataSyncronization = () => {
  const { moduleSyncMap, isGlobalSyncing, runDataSyncronization } =
    useSyncDataStore(
      useShallow(({ moduleSyncMap, isGlobalSyncing, runDataSyncronization }) => ({
        moduleSyncMap,
        isGlobalSyncing,
  ...
  const isSyncing: boolean = useMemo(() => {
    ...
    return values.some((value) => value.syncing) || isGlobalSyncing;
  }, [moduleSyncMap, isGlobalSyncing]);
  ...
  }, [userProfile, isAllSynced, isGlobalSyncing]);
